...
- **Replay.** Press play and the drawing redraws itself stroke by stroke, from 0.1x up to normal speed on a slider that snaps to each step, so someone can watch how it was made. The canvas locks while it plays, and the slider stays on screen so the pace can be changed mid-playback.
